...
// getEncryptionKey checks for IBMCloudCSIDriverConfigSpec in the ClusterCSIDriver object.
...
func getEncryptionKey(ccdLister oplisterv1.ClusterCSIDriverLister) csistorageclasscontroller.StorageClassHookFunc {
	return func(_ *opv1.OperatorSpec, class *storagev1.StorageClass) error {
		...
		if crn == "" {
			klog.V(4).Infof("Not setting empty %s parameter in StorageClass %s", encryptionKey, class.Name)
			return nil
		}
		...
		klog.V(4).Infof("Setting %s = %s in StorageClass %s", encryptionKey, crn, class.Name)
		class.Parameters[encryptionKey] = crn
		return nil
	}
}
